...
const char* wifi_ssid = "iPhone pied";
const char* wifi_password = "collagecoca";
...
void setup() {
  ...
  if (mqtt_client.connect(client_id, mqtt_user, mqtt_pass)) {
    ...
    char temp_str[8];
    char hum_str[8];
    dtostrf(temp_measure, 4, 2, temp_str);
    dtostrf(relative_humidity_measure, 4, 2, hum_str);
    
    // Publish temperature
    if (mqtt_client.publish("TD02_GP03/temp", temp_str)) {
      Serial.print("Temperature published: ");
      Serial.println(temp_str);
    }
    
    // Publish humidity
    if (mqtt_client.publish("TD02_GP03/relhum", hum_str)) {
